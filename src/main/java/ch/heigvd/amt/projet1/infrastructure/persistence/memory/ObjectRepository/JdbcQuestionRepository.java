...
public class JdbcQuestionRepository implements IQuestionRepository {
    ...
    public List<Question> FilterfindPageQuestion(int currentPage, int nbPerPage, List<String> Tags) {
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("SELECT DISTINCT * FROM Question DESC WHERE"+ Filter +"ORDER BY date LIMIT "+Integer.toString(start)+", " + Integer.toString(nbPerPage));
            ResultSet rs = ps.executeQuery();
